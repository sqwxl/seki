...
check_feature_branch() {
    ...
    if [[ "$is_sequential" != "true" ]] && [[ ! "$branch" =~ ^[0-9]{8}-[0-9]{6}- ]]; then
        echo "ERROR: No active Speckit feature directory resolved for current branch: $raw" >&2
        echo "Run /speckit-specify first so .specify/feature.json points to specs/<number>-<feature-name>." >&2
        return 1
    fi
