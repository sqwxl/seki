...
A registered player can create or accept a ranked game, complete it normally, and see both players' ratings and public ranks update from the result.
...
**Independent Test**: Can be fully tested by having two registered users opt into ranking, play a ranked game to resignation, and verify that the winner gains rating, the loser loses rating, and the completed game remains marked as ranked.

**Acceptance Scenarios**:

1. **Given** two registered rating participants with established ratings, **When** they complete a ranked game by resignation, **Then** both ratings are recalculated from the game result and their current rank qualifiers reflect the new ratings.
...
3. **Given** a ranked game has already been created, **When** spectators join, leave, or reconnect, **Then** the game's ranked status and rating eligibility do not change.

---

### User Story 2 - Understand Rank Everywhere (Priority: P2)

Players and spectators can recognize each registered player's ranking status from shared user labels on profiles, game lists, player panels, game-info popovers, and challenge selection.
...
**Independent Test**: Can be tested by viewing ranked, unranked, non-participating, anonymous, and bot users across profile, game list, player panel, and challenge UI contexts.

**Acceptance Scenarios**:

1. **Given** a registered user has a current rating, **When** their label appears in a supported context, **Then** the label includes their current rank qualifier.
2. **Given** a registered user is not yet ranked, **When** their label appears, **Then** the label uses the unranked qualifier.
3. **Given** an anonymous user appears in the same contexts, **When** their label is rendered, **Then** no rank qualifier is shown.
4. **Given** a completed game's history row is viewed later, **When** the players' current ranks have changed since the game began, **Then** the game-bound rank qualifiers still reflect the ranks captured for that game context.
...
A player can rely on their rating history being preserved over time, so future analysis features can show rating progression, rating graphs, and per-game changes.
...
**Independent Test**: Can be tested by completing multiple ranked games, then verifying the player profile can expose each rating change in chronological order with prior rating, new rating, and game reference.
...
3. **Given** a future analysis view requests rating-over-time data, **When** historical rating adjustments exist, **Then** the data is sufficient to reconstruct the player's rating progression without inferring it from current rating alone.
...
- **FR-001**: The system MUST let registered users participate in ranking and clearly distinguish ranked, unranked, and non-participating registered users in visible labels.
...
- **FR-006**: The system MUST capture the relevant player rank qualifier for each ranked or rank-displaying game context so historical game rows remain stable even if current ratings later change.
- **FR-007**: The system MUST update player ratings exactly once when an eligible ranked game reaches a rated terminal result.
- **FR-008**: The system MUST preserve a chronological rating history for each player, including every applied rating adjustment rather than only the current rating.
- **FR-009**: Each rating-history entry MUST include enough user-facing information to explain the change, including the related game, result, rating before the game, rating after the game, and when the adjustment occurred.
- **FR-010**: The system MUST avoid rating changes for unrated games, private games, aborted games, declined games, unplayed games, estimates, analysis-only outcomes, and spectator activity.
- **FR-011**: The system MUST display rank qualifiers using the existing product meanings: `(?)` for registered users without enough rating history, `(-)` for registered users not participating in ranking, and the player's rank for ranked registered users.
- **FR-012**: The system MUST show rank qualifiers in game-info popovers, player panels, game lists, player profile titles, and challenge-user selection.
- **FR-013**: The system MUST show unrated status in the game-info popover and append `(unrated)` to unrated game descriptions in game lists.
- **FR-014**: The system MUST allow users browsing games to filter by rated/unrated status and by a practical rank range for available opponents.
- **FR-015**: The system MUST show a player's current rating/rank summary and rated-game history on their profile when the viewer is authorized to see the relevant games.
- **FR-016**: The system MUST keep rating history attached to the user identity across username changes and registration upgrades from anonymous sessions.
- **FR-017**: The system MUST provide user-facing feedback when a ranked-game action is unavailable because of registration, participation, privacy, or opponent eligibility rules.
- **FR-018**: The system MUST handle bot users as eligible ranked participants while preserving bot-specific social restrictions.
...
- **Rating Profile**: The ranking state for a registered or bot user, including participation status, current rating, current rank qualifier, provisional/unranked state, and summary counts.
- **Rated Game Snapshot**: The rating-relevant facts captured for each eligible game, including ranked status, player identities, game-bound rank qualifiers, result category, and whether rating updates have been applied.
- **Rating Adjustment**: A durable history record of a completed rated result's effect on each participant, including prior rating, new rating, opponent context, result, adjustment time, and game reference.
- **Rank Qualifier**: The user-facing label derived from rating participation and rating strength, displayed consistently across game, profile, and challenge contexts.
...
- **SC-003**: 100% of applied rating updates create a corresponding historical adjustment entry with enough information to reconstruct each player's rating progression.
...
- **SC-005**: 95% of users in usability checks can correctly identify whether a visible game is ranked or unrated and whether a visible player is ranked, unranked, or not participating.
- **SC-006**: Game-list filtering by rated status and rank range returns matching visible games within 1 second for typical lobby sizes.
- **SC-007**: Existing authorized profile and game-history visibility rules remain unchanged for private and invite-protected games in access-control tests.

## Assumptions

- The first version uses an Elo-style numeric rating mapped to kyu/dan-style display ranks, because the README explicitly calls out "ELO, kyu/dan".
...
- Users without enough completed ranked games are shown as unranked until they meet the product's minimum history threshold.
